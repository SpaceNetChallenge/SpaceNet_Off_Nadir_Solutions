...
def train_epoch(current_epoch, loss_function, l1_loss, model, optimizer, scheduler, train_data_loader):
    ...
    for i, sample in enumerate(iterator):
        ...
        loss = loss1 + 0.2 * loss2 + 0.02 * loss3 + 0.002 * loss4 + 0.0002 * nadir_loss #  
...
if __name__ == '__main__':
    ...
    best_snapshot_name = 'res101_9ch_fold_{0}_best_0'.format(fold)
    last_snapshot_name = 'res101_9ch_fold_{0}_last_0'.format(fold)

    np.random.seed(1)
    random.seed(1)

    train_files = df[~df['fold'].isin([fold, fold+4])]['id'].values
    ...
    optimizer = AdamW(params, lr=2e-4, weight_decay=1e-5)
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[2, 7, 17, 23, 28, 32], gamma=0.5)
    loss_function = ComboLoss({'dice': 1.0, 'focal': 10.0}, per_image=True).cuda() #dice
    ...
    for epoch in range(25):
        if epoch < 1:
            for p in model.module.conv1.parameters():
                p.requires_grad = False
            for p in model.module.conv2.parameters():
                p.requires_grad = False
            for p in model.module.conv3.parameters():
                p.requires_grad = False
            for p in model.module.conv4.parameters():
                p.requires_grad = False
            for p in model.module.conv5.parameters():
                p.requires_grad = False
        else:
            for p in model.module.conv1.parameters():
