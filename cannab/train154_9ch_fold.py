...
def train_epoch(current_epoch, loss_function, l1_loss, model, optimizer, scheduler, train_data_loader):
    ...
    for i, sample in enumerate(iterator):
        ...
        loss = loss1 + 0.5 * loss2 + 0.1 * loss3 + 0.001 * loss4 + 0.0005 * nadir_loss #  
...
if __name__ == '__main__':
    ...
    best_snapshot_name = 'res154_9ch_fold_{0}_best_0'.format(fold)
    last_snapshot_name = 'res154_9ch_fold_{0}_last_0'.format(fold)
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[1, 3, 10, 17, 20], gamma=0.5)
